...
with sync_playwright() as p:
    ...
    check('行1含74.9%', '74.9%' in core_txt and '77.08万' not in core_txt, core_txt[:200])
    hold_txt = page.locator('#t0GridHold').inner_text()
    check('行2含3.7%超额', '相对满仓' in hold_txt and '3.7%' in hold_txt, hold_txt[:200])
    t0g_txt = page.locator('#t0GridT0').inner_text()
    check('行3含19.0%超额', '相对做T' in t0g_txt and '19.0%' in t0g_txt, t0g_txt[:200])
