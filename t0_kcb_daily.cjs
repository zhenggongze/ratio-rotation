// 科创50(588000) 做T每日记录脚本
// 用法:
//   node t0_kcb_daily.cjs              # 拉腾讯当日1分钟K线，判定当日记录（16:00后运行）
//   node t0_kcb_daily.cjs --init       # 用本地 pytdx 5分钟数据(2024-07~2026-08) 初始化历史每日记录
// 输出: data/t0/t0_daily_kcb.json（按 date 幂等覆盖，与红利 t0_daily.json 结构一致）
const fs = require('fs');
const path = require('path');
const https = require('https');
const { KCB_CONFIG, computeMinuteDailyKcb } = require('./t0_engine_kcb.cjs');

const DAILY_FILE = path.join(__dirname, 'data', 't0', 't0_daily_kcb.json');
...
function fetchM1() {
  return new Promise((resolve, reject) => {
    const url = `https://ifzq.gtimg.cn/appstock/app/kline/mkline?param=${KCB_CONFIG.SYMBOL},m1,,320`;
    https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://gu.qq.com/' } }, res => {
      ...
      res.on('end', () => {
        try {
          ...
          const m1 = json.data && json.data[KCB_CONFIG.SYMBOL] && json.data[KCB_CONFIG.SYMBOL].m1;
...
function initFromLocal() {
  ...
  if (!fs.existsSync(src)) { console.log(`✗ 本地5分钟数据不存在: ${src}`); process.exit(1); }
  ...
  for (let i = 0; i < dayList.length; i++) {
    ...
    const rec = computeMinuteDailyKcb(d, bars, i > 0 ? closes[dayList[i - 1].d] : null);
...
async function updateDaily() {
  ...
  if (bjHm < 15 * 60 + 30) {
    ...
    const factor = 10 ** KCB_CONFIG.PRICE_DECIMAL;
    const buyP = Math.floor(open * KCB_CONFIG.BUY_K * factor) / factor;
    const sellP = Math.floor(open * KCB_CONFIG.SELL_K * factor) / factor;
